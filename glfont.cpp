...
glFont::glFont() : bakedFont("Baked Font") {
  pixelHeight = 32.0f;
  codePointStart = 32;
  codePointEnd = 96;
  cdata = nullptr;
...
void glFont::Init(const char *filename) {
  ...
  stbtt_BakeFontBitmap(&ttf_buffer[0], 0, pixelHeight, bakedFont.imgData,
                       bakedFont.width, bakedFont.height, codePointStart,
                       codePointEnd,
...
GLuint glFont::GenBindVertList(float x, float y,
  ...
  for (const char &c : text) {
    ...
    stbtt_GetBakedQuad(cdata_typed, bakedFont.width, bakedFont.height, c, &x,
                       &y, &q,
                       1); // 1=opengl
