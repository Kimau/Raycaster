...
void ProgramData::create() { progID = glCreateProgram(); }
...
ProgramData *ProgramData::GetProg(int ID) { return &(g_progArray[ID]); }
...
void unbindProgram() { glUseProgram(NULL); }

GLuint compileShader(const GLchar *source, GLenum type) {
  GLuint shader = glCreateShader(type);
  glShaderSource(shader, 1, &source,
                 NULL); // One Shader Source which is NULL terminated
...
void ShaderUniform::Set(int x, int y, int z) { glUniform3i(loc, x, y, z); }

void ShaderUniform::Set(int x, int y) { glUniform2i(loc, x, y); }

void ShaderUniform::Set(int x) { glUniform1i(loc, x); }
...
void ShaderUniform::SetF(float x, float y) { glUniform2f(loc, x, y); }

void ShaderUniform::SetF(float x) { glUniform1f(loc, x); }

void ShaderUniform::SetMatrix2(const float *val, int count /*= 1*/,
                               bool transpose /*= false*/) {
  glUniformMatrix2fv(loc, count, transpose, val);
}

void ShaderUniform::SetMatrix3(const float *val, int count /*= 1*/,
                               bool transpose /*= false*/) {
  glUniformMatrix3fv(loc, count, transpose, val);
}

void ShaderUniform::SetMatrix4(const float *val, int count /*= 1*/,
                               bool transpose /*= false*/) {
...
void ShaderAttrib::EnableArray() { glEnableVertexAttribArray(loc); }

void ShaderAttrib::DisableArray() { glDisableVertexAttribArray(loc); }

//////////////////////////////////////////////////////////////////////////

void genBasicFloatNBuffer(GLuint index, ShaderAttrib &attrib, GLfloat *vertData,
                          GLuint length, GLuint numFloatrs) {
  ...
  glBufferData(GL_ARRAY_BUFFER, length * numFloatrs * sizeof(GLfloat), vertData,
